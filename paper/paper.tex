...
downloaded this metadata for all 797,940 packages on the \code{npm} registry, having found the names of all packages using the \code{npm} registry skimdb
...
we care about for this study. There are two kinds of dependencies in the metadata files, a list of runtime dependencies and \code{devDependencies} used in testing and development.\\

We gathered some information on download counts for \code{npm} packages from the \code{npm} API\footnote{https://api.npmjs.org/downloads/range/2010-01-01:2019-04-01/packageName} 
