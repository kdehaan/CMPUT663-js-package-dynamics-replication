-- Counts how many projects resolved to *any* version of a package when it
-- came out. (for now, only express, and that took 15mn)
CREATE TABLE `resolvable_dependents`
SELECT
    `p`.`id` AS `package_id`,
    -- `p`.`name` AS `package_name`,
    `pv`.`version` AS `version`,
    `pv`.`released` AS `released`,
    COUNT(*) AS `resolvable_dependents`
